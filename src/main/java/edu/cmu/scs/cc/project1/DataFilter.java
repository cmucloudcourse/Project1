...
public final class DataFilter {
    ...
    static boolean checkPrefix(final String[] columns) {
        String decodedTitle = PercentDecoder.decode(columns[TITLE]);
        return !Arrays.stream(PREFIX_BLACKLIST).anyMatch(decodedTitle.toLowerCase()::startsWith);
    ...
    static boolean checkFirstLetter(final String[] columns) {
        ...
        for(int i = 0 ; i <26 ; i++){
            alphabets.add((char)(alphabet+i)) ;
        }
        char firstChar = columns[TITLE].charAt(0);
        return ! alphabets.contains(firstChar);
    }
}
